...
// init time module
var renderer = initTime(13, 12, 15, 1);
updateTime();

function dateToLocal(date, i = 8) {
...
function getTime() {
  const startTime = dateToLocal(new Date(window.startTime));
  const now = dateToLocal(new Date());
  const latestDay = dateToLocal(new Date(startTime.getFullYear(), startTime.getMonth() + 1, 0)).getDate();

  const result = {};
  const up = [ 0, 12, latestDay, 24, 60, 60 ];
  const methods = [ 'getFullYear', 'getMonth', 'getDate', 'getHours', 'getMinutes', 'getSeconds' ];
  const props = [ 'year', 'month', 'day', 'hour', 'minute', 'second' ];

  let d = 0;
  while (up.length) {
    const index = up.length - 1;
    const upper = up.pop();
    const method = methods[index];
    const prop = props[index];

    // calculate time
    result[prop] = now[method]() - startTime[method]() + d;
    if (result[prop] < 0) {
      result[prop] += upper;
      d = -1;
    } else {
...
function updateTime() {
  ...
  renderer.render(
    [
      '已经',
      timeObj.year ? '{{ year }} 年' : null,
      timeObj.month ? '{{ month }} 个月' : null,
      '{{ day }} 天',
      '{{ hour }} 小时',
      '{{ minute }} 分',
      '{{ second }} 秒了'
